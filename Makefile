...
help:
	@printf '%s\n' \
		...
		'  check-matching-hygiene  Reject matching C that depends on register pins, inline asm or mixed -G' \
...
check-metadata:
	...
	@$(PYTHON) tools/project/check_matching_hygiene.py

check-matching-hygiene:
	@$(PYTHON) tools/project/check_matching_hygiene.py
