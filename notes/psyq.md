...
| `RECT` in `src/psyq/libgpu.h` | GPU transfer rectangle | Typed migration is established in `Duel_SetupCardRecord`, the native local `RECT` in [`main_menu_load_package_stage.c`](../src/game/main_menu_load_package_stage.c), and `func_80057544`/`func_800577B0` in [`model_texture_transfer.c`](../src/game/model_texture_transfer.c); preserve byte-offset selection and layout-compatible casts elsewhere when exact code generation requires them. |
