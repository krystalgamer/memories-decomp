...
- Explore as many materially distinct source/compiler variants for one function
  as the evidence supports; there is no attempt ceiling. Record every result in
  `config/slus_01411/attempts.csv`, and defer a function only when its evidence
  is exhausted rather than when a counter runs out.
