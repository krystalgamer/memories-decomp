...
**The `func_800179F4` coalescing lever does not transfer here.** That function
had the same shape -- retail completing a `%hi`/`%lo` pair in place where the
build routed it through a scratch register -- and was closed by an intermediate
local pinned to the target register. Here that does nothing: `register Slot *slot
__asm__("$16")`, a separate pinned `base` feeding `slot`, and an unpinned `base`
feeding `slot` all measure the same 8. Nor does the addressing form, which is
the other half of that lever: a `section(".data")` **scalar** with its address
taken, a plain scalar with its address taken, and the array carrying the section
attribute are all 8 as well, and the emitted pair stays `lui $v0` /
`addiu $s0, $v0` in every one.

So the two levers that look most applicable are both spent. What is left is
narrow: the `HIGH` pseudo has two references and a one-instruction live range,
which under the GCC 2.8.1 allocno ranking is about as high a priority as a
pseudo can have, so it is allocated early and takes `$v0` as the first free
register. Retail has it in `$s0`, the same register as the `lo_sum` result.
`lo_sum` is not a copy insn, so nothing preferences the two together, and no
source spelling tried so far reaches the allocator's choice. The next attempt
should look for something that lowers that pseudo's priority or lengthens its
live range, not for another way to spell the symbol.
