...
**The `$s0` pin does fix the coalescing -- it is the *placement* that is left.**
This corrects an earlier reading of my own. Measuring only the differing-word
count says the pin is inert, because it stays at 8. Reading the emitted window
says otherwise:

    target                   unpinned                 register slot __asm__("$16")
    sh    $a0, %gp_rel(..)   lui   $v0, %hi(D)        sw    $s0, 0x20($sp)
    move  $a0, $zero         sw    $s0, 0x20($sp)     lui   $s0, %hi(D)
    move  $a1, $a0           addiu $s0, $v0, %lo(D)   addiu $s0, $s0, %lo(D)
    lui   $v0, %hi(fn)       sh    $a0, %gp_rel(..)   sh    $a0, %gp_rel(..)
    addiu $v0, $v0, %lo(fn)  move  $a0, $zero         move  $a0, $zero
    sw    $s0, 0x20($sp)     move  $a1, $a0           move  $a1, $a0
    lui   $s0, %hi(D)        lui   $v0, %hi(fn)       lui   $v0, %hi(fn)
    addiu $s0, $s0, %lo(D)   addiu $v0, $v0, %lo(fn)  addiu $v0, $v0, %lo(fn)

Pinned, the pair is `lui $s0` / `addiu $s0, $s0` -- retail's coalesced form, the
thing this entry identifies as the blocker. The instruction multiset is then
identical to the target and the two builds differ only by a **rotation**: the
three-instruction `D_800EAE98` group sits at the front instead of the end.

So the entry's causal claim -- "fix the register and the order should follow" --
is wrong in one direction. The register is fixable and the order does *not*
follow. But the residual is now much narrower than "eight positions": it is
where one three-instruction group sits, with every instruction already correct.

Crossed on the pinned base without moving the rotation: six placements of the
`slot = D_800EAE98` assignment (four stay at 8, moving it past
`File_RequestAsyncTransfer` costs 16, past `func_80039E9C` costs 17).

**What does not transfer is the *addressing* half of the `func_800179F4` lever.**
A `section(".data")` scalar with its address taken, a plain scalar with its
address taken, and the array carrying the section attribute all leave the pair
uncoalesced at `lui $v0` / `addiu $s0, $v0`. Only the pin reaches it.

**The rotation is a `sched2` placement with no source handle found.** On the
pinned base, where the instructions are already all correct, three further
sweeps leave it unmoved:

- **All 31 profiles.** The entry's sweep was on the unpinned base, which is a
  different starting point, so it was worth redoing. Only the four `g8_split`
  variants reach 8; every other profile is 49 or worse.
- **The six placements, read at window level rather than by count.** All four
  that stay at 8 emit a *byte-identical* window, so the scheduler normalises the
  statement position away entirely rather than trading one difference for
  another.
- **The callback address through a named local**, assigned before `slot` and
  after the `D_8009B2A4` store: 8 both ways.

Reading the two orders against each other, the callback pair feeds an argument
of the pending call and the `D_800EAE98` pair feeds nothing until after it, so
on a critical-path model the callback pair should sort first, which is retail.
This build sorts the `D_800EAE98` group first anyway, and the group moves as a
unit with the `sw $s0` prologue save that has to precede its clobber of `$s0`.
Nothing tried reaches that ordering decision.
