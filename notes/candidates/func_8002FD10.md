...
**Why the temporary lands in `$v0`, and why that is hard to move.** GCC does
coalesce this form when it can: in the same prologue the callback address comes
out `lui $2, %hi(func_8002FB78)` then `addiu $2, $2, %lo(...)`, one register.
The `D_800EAE98` pair does not, because its destination is `$s0` -- `slot`
lives across calls -- and local-alloc gives the short-lived `HIGH` temporary the
first free hard register in allocation order, which is `$v0`. In the target the
callback address is already sitting in `$v0` when the pair is emitted, so the
temporary cannot have it and falls to `$s0`, where it coalesces with the
destination. The register and the order are one fact, not two.

Crossed since, none better than 8: the `slot` assignment moved to five
positions through the prologue, from immediately after `D_8009B2A4 = arg0` down
to after `func_80039E9C()` -- the first three are inert at 8 and the last two
are worse at 17 and 43, so the scheduler hoists the pair regardless of where it
is written. Holding the callback in a local assigned before `slot` does not
help either, plain or pinned to `$2`; GCC coalesces the local with the address
materialisation and the live range does not extend the way it needs to.
Pinning `slot` itself is inert -- it is already `$s0` -- and pinning it
anywhere else is much worse, 36 for `$17` and 42 for `$18`.
