...
not for two scalar stores. Writing `a[i].x = ...; a[i].y = ...;` produces two
stores and no copy; the target's four extra `lw`/`sw` per pair only appear if
the source assigns a two-word structure at once. So the local is an array of a
two-word struct and each element is assigned as a unit.
