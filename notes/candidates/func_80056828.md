## `func_80056828` at 0x80056828

`gcc_2_8_1_g8_no_split`, 341 instructions against a target of 341, opcode
distance 24 and 291 differing linked words.

*Distance verified against encodings.* A histogram keyed on printed mnemonics
reports 22 for this entry rather than 24, because `move` and `li` are
...
encoding-keyed one and is correct; it should not be "corrected" to 22.

This is the complete eleven-state per-player model dispatcher. It replaces the
75-instruction automated sketch with every target case represented and preserves
the target's seven saved registers, 56-byte frame, GP-relative player byte,
callback entry-point adjustment, and case-8 register assignment.

The largest remaining differences are source shape in cases 0 and 3, the
state-to-jump-table dispatch schedule, and common-tail scheduling. The case-8
path now has the target instruction count and register roles; keep its pinned
`s1`, `s2`, `s3`, and `s5` locals when refining the other regions.
