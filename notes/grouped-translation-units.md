...
| `src/game/sound_voice_selection.c` | `gcc_2_8_1_g0` | The SPU voice-slot selection unit, eight contiguous functions from `0x800478EC` through `0x80047CC4`: the per-slot envelope/timer sweep that keys voices off, the threshold key-off, the two request entry points and the list variant, the sign-bit id normaliser (`0x80047C50`), the bounded key-off retry (`0x80047C70`) and the round-robin slot release (`0x80047CC4`). All eight are declared in `sound_voice_selection.h`, which already covered them while they sat in three sources. #3859 moved `func_80047864`, `func_80047C50` and `func_80047DB0` to `src/candidates/` and the unit was split around them; #4025 matched `func_80047C50` again without register pins and returned it to `src/game/`, which made the run contiguous once more without rejoining the sources. This is that rejoin. Bounded below and above by the two functions still in `src/candidates/` |
| `src/game/sd_key_off_voice_slots.c` | `gcc_2_8_1_g0` | 2 contiguous functions: `SD_KeyOffVoiceSlots` (`0x80047EC4`), `0x80047F38`. Split out of `src/game/sound_voice_selection.c` after #3859 moved `func_80047864` and `func_80047DB0`, which matched only through pinned registers or inline asm, to `src/candidates/`. |
