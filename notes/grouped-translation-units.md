...
| `src/game/mdec_sync.c` | `gcc_2_8_1_g8` | Movie fade-out request `Movie_StartFadeOut` (`0x8005C5C4`), which sets the `D_8009B064` flag rather than the `D_8009B062` completion latch, and the contiguous bounded wait/reset helper `Movie_WaitFrameDecoded` (`0x8005C5D4`) that clears that latch |
