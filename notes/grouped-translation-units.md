...
| `src/game/duel_effect_object_commands.c` | `gcc_2_8_1_g8` | Five contiguous secondary duel-effect stream commands from `0x800389C4` through `0x80038B08`: clear a channel flag, update either half of the current stream word, unlock a defeated duelist, and retire the channel's display object. They occupy one command-table run and share the channel stream/state records. Bounded above by `text_control_commands.c` at `gcc_2_8_1_g8_split` |
