...
| `src/game/duel_draw_status_numbers.c` | `gcc_2_8_1_g8_split` | The duel screen's life-point and deck-count readout: the digit-sprite row `func_80016D2C` (`0x80016D2C`), `Duel_UpdateLifePointDisplay` (`0x80016DDC`), which steps the drawn counter towards the real total, and `Duel_DrawLifePointsAndDeckCounts` (`0x80016E70`), which calls both. The first two were recorded at `gcc_2_8_1_g0_split` and `gcc_2_8_1_g8` and compile to identical objects at `gcc_2_8_1_g8_split`. Bounded below by the card-frame draw callbacks and above by the card-record helper `func_8001700C` |
...
| `src/game/fade_overlay.c` | `gcc_2_8_1_g8_split` | The fade overlay and both sides of its setup, ten contiguous functions: `Fade_DrawOverlay` (`0x800154E4`), the band fill `func_800156B8` (`0x800156B8`), the white-mode reset (`0x800156DC`) and white fade-in helper (`0x8001572C`), uniform `Fade_InitIn` (`0x80015780`, target `0xFF`, flag `0x80`, step `0x0C`) and banded `Fade_StartIn` (`0x800157DC`, flag `0x01`, step `8`), colour-selecting `Fade_InitInColor` (`0x8001581C`, flags `0x30`) with the conditional white-state reset (`0x80015870`, flags `0xB0`, step `0x0C`), and the fade-out pair `Fade_InitOut` (`0x800158B8`, head `0xFF`, target `0`, flag `0x80`, step `0x0C`) and contiguous strip-mode `Fade_StartOut` (`0x80015904`, flag `0x01`, step `8`). The seven former sources were recorded at `gcc_2_8_1_g8_split`, `gcc_2_8_1_g0_split` and `gcc_2_8_1_g8`, and every member compiles to an identical object at `gcc_2_8_1_g8_split` -- including the fade-out pair, which this table read as a profile boundary until it was measured. Bounded below by `Fade_Update` (`gcc_2_8_1_cc_g8_as_g1_split_comm`) and above by `fade_control.c`, the blocking and wrapper layer built on these primitives, which is a boundary of meaning rather than of profile |
| `src/game/fade_control.c` | `gcc_2_8_1_g8_split` | Colour-selecting `Fade_InitOutColor` (`0x80015944`, flags `0x30`), the blocking frame-pump loop `Fade_Wait` (`0x80015998`) that waits for flag `0x80` to clear, and the eighteen contiguous blocking/nonblocking fade wrappers built on them and the other setup paths, direct level/target setters, flag-`0x02`/`0x06` variants, and overlay-latch toggles from `0x800159D8` through `0x80015D0C`, including `Fade_WaitOut`. The wrappers were recorded at `gcc_2_8_1_g8` but compile to identical objects at `gcc_2_8_1_g8_split`. Bounded below by `fade_overlay.c`, which holds the `Fade_InitOut` and `Fade_StartOut` these wrappers call and is a boundary of meaning rather than of profile, and above by `display_projection.c` |
...
| `src/game/duel_effect_command.c` | `gcc_2_8_1_g8_split` | Entries 0 through 12 of the secondary text-command table `D_80090EAC`, the handlers the F8 escape reaches, from `func_80037DA4` (`0x80037DA4`) through `func_800384E4` (`0x800384E4`), with their shared helper `func_80038024`: fourteen contiguous functions. The eight former sources were recorded at four profiles, and every member compiles to an identical object at `gcc_2_8_1_g8_split`. Bounded below by `text_stream_commands.c` and above by entry 13, `Text_StartCampaignDuel`, which builds only at `gcc_2_8_1_g0` |
...
| `src/game/duel_rewards.c` | `gcc_2_8_1_g8_split` | The duel's reward step, five contiguous functions: the result-page switch `Duel_ShowResultPage` (`0x80021480`), the per-rule lookup `Duel_CalcRankScoreChange` (`0x80021558`), `Duel_CalcRankScore` (`0x80021598`), which sums those into the display's side scores, `Duel_SelectCardDrop` (`0x80021810`) and `Duel_AwardCard` (`0x80021894`). The first two were recorded at `gcc_2_8_1_g8` and `gcc_2_8_1_g0_split` and compile to identical objects at `gcc_2_8_1_g8_split`. Bounded below by the duel-result outro in `duel_result_outro.c` and above by unmatched assembly |
