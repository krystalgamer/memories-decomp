...
| `src/game/card_list_sort.c` | `gcc_2_8_1_g8_split` | Four contiguous card-list functions from `BuildDeck_CompareCard` (`0x80032B60`) through the cursor/page input handler `func_800330BC` (`0x800330BC`), spanning 0x9A0 bytes. The two comparators order equal-key rows by `gCard_asNameSortKey`; `func_80032C48` builds per-row keys and dispatches `qsort`; the final handler scrolls and pages the list or changes the sort mode before invoking that sorter. The comparators were recorded at `gcc_2_8_1_g0_split` and compile identically at the unit's G8 split profile, while the handler's explicit volatile `.data` input views retain its absolute reloads. The transition latch `func_80032B38` below and the non-split `func_80033500` above fix both boundaries. |
...
| `src/game/duel_field_equip_search.c` | `gcc_2_8_1_g8_split` | Four contiguous field/equip search helpers from the empty-slot scan `func_80026C0C` (`0x80026C0C`) through the equip-pair planner `func_80026DC8` (`0x80026DC8`), spanning 0x454 bytes. The two middle collectors build null-terminated lists of occupied cards below or equal to a requested type, and the planner consumes those lists together with the empty-slot result to populate `D_800EAE88`. This restores the complete unit after #3859 temporarily demoted the first two functions. The magic-effect activation helper below uses the same profile but belongs to the preceding dispatch unit; `func_80027060` above changes to G0 split. |
