...
| `src/game/model_primitive_handler.c` | `gcc_2_8_1_g0_split` | Primitive-family selector (`0x800603DC`), `Model_GetPrimitiveHandler` (`0x8006041C`), and contiguous `func_8006086C` (`0x8006086C`), which is the only caller of `Model_GetPrimitiveHandler` in the tree: it stores the handler that call returns through the object's own pointer and registers it under the same key. Producer and sole consumer. The three sizes sum to `0x4DC`, so the run reaches `func_800608B8`. `func_8006086C` was recorded at `gcc_2_8_1_g0` and compiles to an identical object at `gcc_2_8_1_g0_split`, which the other two already use. `model_handler_registry.c` at `gcc_2_8_1_g8_split` bounds it below; above, `func_800608B8.c` is at the same `gcc_2_8_1_g0_split`, so no profile change bounds it there -- `func_800608B8` is the selector for a second handler table and `func_80060AEC` its binder, the same two-function shape as this unit, and both are left as is |
