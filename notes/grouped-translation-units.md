...
| `src/game/duel_effect_command.c` | `gcc_2_8_1_g8_split` | Complete contiguous secondary text-command run from helper `func_80038024` through table entry `func_800384E4`: 13 functions spanning `0x80038024..0x80038530`. The twelve handlers are entries 1 through 12 of `D_80090EAC` and all consume or update the same `DuelEffectChannel` stream state. Entry 0, `func_80037DA4`, remains a candidate below the run; entry 13, `Text_StartCampaignDuel`, uses `gcc_2_8_1_g0` above it. The four-source split created by #3859 is closed after pure-C recovery of `func_800382A8` and `func_80038334`. |
