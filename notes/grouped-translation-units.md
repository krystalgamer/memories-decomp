...
| `src/game/movie_frame_pipeline.c` | `gcc_2_8_1_g8` | Four contiguous movie-stream functions: `Movie_StopStream` (`0x8005BB7C`), `Movie_DecodeAndPresentFrame` (`0x8005BE3C`), `Movie_WaitAndDecodeFrame` (`0x8005BFC8`), and the MDEC strip callback `func_8005C1F4` (`0x8005C1F4`). #3859 moved `func_8005C1F4`, which matched only through pinned registers, to `src/candidates/`; it is back as pure C (#5). |
