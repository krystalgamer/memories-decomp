...
| `src/game/sound_mix.c` | `gcc_2_8_1_g0` | 2 contiguous functions: `0x80044F58`, `0x80044FE4`. What remains after #3859 moved `SD_SetCdPan`, which then matched only through pinned registers or inline asm, to `src/candidates/`; it has since matched in pure C in its own `sd_set_cd_pan.c`. |
