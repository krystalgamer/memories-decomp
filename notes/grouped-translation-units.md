...
| `src/game/duel_draw_status_numbers.c` | `gcc_2_8_1_g8_split` | 3 contiguous functions: `0x80016D2C`, `Duel_UpdateLifePointDisplay` (`0x80016DDC`), `Duel_DrawLifePointsAndDeckCounts` (`0x80016E70`). The third function is restored from the candidate store by its pure-C scratch-packet initialization order. |
...
| `src/game/display_parent_links.c` | `gcc_2_8_1_g8` | Contiguous display-parent helpers: `func_80022F98` (`0x80022F98`) attaches one child object to the parent's position base and callback, then `func_80022FF0` (`0x80022FF0`) applies it to the base and both objects in each of five field rows, optionally clearing the stored pointers. Both share `DisplayParent`/`DisplayLinkEntry` and the latter calls the former directly. |
| `src/game/duel_field_display_objects.c` | `gcc_2_8_1_g8_split` | 5 contiguous functions: `0x80023090`, `0x80023144`, `0x8002348C`, `0x800234E4`, `0x800235C0`. #3859 moved `func_80023090` and `func_80023144`, which matched only through pinned registers or inline asm, to `src/candidates/`; both are back as pure C (#5). `func_800235C0` was decompiled fresh (#5). |
...
| `src/game/func_8002C938.c` | `gcc_2_8_1_g0_split` | Two contiguous zero-terminated duel display-object collectors at `0x8002C938` and `0x8002C9B4`. The first selects one active-side row and emits every occupied object; the second selects both side zones, an object subtype, or a minimum calculated attack threshold. Both traverse `D_801A7AD8`/`D_801A7B64` as `DuelCardRecord` storage and publish their contracts through `duel_card.h`. |
...
| `src/game/ai_script_vm.c` | `gcc_2_8_1_g0_split` | The AI script VM and its stream fetch primitives, four contiguous functions: `AiScript_ReadByte` (`0x8007058C`) and `AiScript_ReadShort` (`0x800705AC`), which advance `gAiScript_State.script_cursor` by one and two bytes, followed by `AiScript_Init` (`0x800705D8`) and `AiScript_Run` (`0x80070650`), which calls the byte reader on every iteration to fetch the next opcode. The three former sources were recorded at three profiles and every member compiles to an identical object at `gcc_2_8_1_g0_split`. Both readers keep their headers, which have consumers outside this unit. Bounded above by `Ai_GetHandSize`, which fills the 40 bytes between this group's end and `ai_card_ranges.c` |
...
| `src/game/ai_script_query_commands.c` | `gcc_2_8_1_g0_split` | The AI script VM's query opcodes, five contiguous functions from `AiScript_TestHighStat` (`0x80071194`) through `AiScript_LoadLifePoint` (`0x800713B8`): each reads an operand byte with `AiScript_ReadByte`, indexes `gAiScript_aMemory` with it, looks game state up in `gDuel_adwCardStats`, `gDuel_aActiveCards` or `D_800E9FF0`, and answers back into `gAiScript_aMemory`. The three former sources were recorded at two profiles and every member compiles to an identical object at `gcc_2_8_1_g0_split`. The unit was `ai_script_card_info.c` and is renamed because two of the five read no card info -- `AiScript_SetFaceDown` classifies a card's visibility and `AiScript_LoadLifePoint` reads a side's life points. Bounded below by the adjacent `ai_script_calc_card_power.c` (`gcc_2_8_1_g8_split`) and above by a 120-byte gap before `ai_script_load_deck_size.c` |
...
| `src/game/ai_script_combo.c` | `gcc_2_8_1_g0_split` | Four contiguous AI script register/combo opcodes from `AiScript_LoadDeckSize` (`0x8007149C`) through `AiScript_StartCombo` (`0x800715C4`): live own-card count minus the five-card hand baseline, the `D_800EAE90` register load at `0x80071510`, pinned-card testing, and combo-card selection. All four use the bytecode operand reader and `gAiScript_aMemory`; the final two additionally consume duel-side or combo state. The unit spans 0x1B0 bytes and is bounded below by the immediately adjacent `ai_script_load_terrain.c` at `gcc_2_8_1_g8_split` and above by `ai_script_load_best_difference.c`, which requires the no-sched1 profile. |
...
| `src/game/ai_script_find_card.c` | `gcc_2_8_1_g0_split` | Four contiguous AI field-search opcodes from `AiScript_FindDefenseStopper` (`0x80071CB0`) through `AiScript_FindCard` (`0x8007214C`), spanning 0x61C bytes. The defense comparison pairs strongest available monsters until the opponent is stopped; `AiScript_CountCards` counts occupied slots in a decoded zone; `AiScript_FindFirstCard` and `AiScript_FindCard` return the first slot matching state or card ID. All four read bytecode operands, scan `gDuel_aActiveCards`, and answer through `gAiScript_aMemory`. The complete unit is restored after #3859 temporarily demoted the intervening count opcode; profile changes to `ai_script_find_killer.c` below and `ai_script_match_type.c` above fix both boundaries. |
...
| `src/game/func_80013C28.c` | `gcc_2_8_1_g8_split` | 5 contiguous asynchronous transfer functions from the ready-sector callback at `0x80013C28` through the four command-completion callbacks ending at `func_80014220` (`0x80014220`). They share the transfer descriptor, state word, counters, and event protocol. |
| `src/game/func_80014294.c` | `gcc_2_8_1_g8_split` | 9 contiguous functions: `0x80014294`, `0x80014308`, `0x80014390`, `File_ActivateTransfer` (`0x800143DC`), `0x800144B8`, `0x8001455C`, `0x80014A5C`, `0x80014B30`, `0x80014C40`. The run follows the five-function asynchronous callback unit in `src/game/func_80013C28.c`. |
| `src/game/file_transfer_flags.c` | `gcc_2_8_1_g8` | Seven contiguous helpers over one `FileTransferDescriptor`: the active/secondary transfer-request and cancellation helpers from `0x80014E1C` through `0x80015078`, and the phase stepper `func_8001513C` (`0x8001513C`) that counts a descriptor's phase down and fires its `phase_callback`. The run this sits in is bounded by a profile change on both sides, `func_80014294.c` (`gcc_2_8_1_g8_split`) below and `Fade_StepBands` above (assembly since #3859; it was `fade_step_bands.c` at `gcc_2_8_1_cc_g8_as_g0_split`), but it is not taken whole: the fade-state reset `func_800151B0` sits between this unit and that upper bound and resets `gFade_State`, so it is left in its own source. It cannot join the fade units either, because they use a different profile |
...
| `src/game/movie_frame_pipeline.c` | `gcc_2_8_1_g8` | Five contiguous movie-stream functions from `Movie_StopStream` (`0x8005BB7C`) through the three-byte movie-mode setter `func_8005C374` (`0x8005C374`), spanning 0x80C bytes. Between them, the decode/present frame pump, CD-ring wait/VLC stage, and MDEC strip callback own the complete frame pipeline. #3859 temporarily moved the strip callback to candidates; its pure-C promotion and the adjacent setter now restore the full G8 run. The preceding stream initializer uses G8 split and the following range request uses G0 split. |
| `src/game/movie_stream_requests.c` | `gcc_2_8_1_g0_split` | Two contiguous movie request constructors totaling 0x168 bytes: `func_8005C388` (`0x8005C388`) selects a configured stream range and converts its accumulated sector offset through `CdlLOC`, while `Movie_StartFileStream` (`0x8005C464`) resolves a named file and requests its sector-aligned extent. Both finish in `func_8005B8A0` and share the direct `file_cd_helpers.h` conversion ownership. G8 movie-frame code below and G0 file/CD helpers above fix the boundaries. |
| `src/game/model_update_view_metrics.c` | `gcc_2_8_1_g8_split` | The two-stage camera-relative model transform, totaling 0x388 bytes: `Model_UpdateViewMetrics` (`0x80057F38`) caches the active `GsRVIEW2` and derives distance, yaw and pitch, then `func_800580D4` (`0x800580D4`) consumes those angles with a selected model entry to build the requested coordinate-unit transform. The adjacent functions on both sides use non-split G8. |
| `src/game/file_cd_helpers.c` | `gcc_2_8_1_g0` | Three contiguous file/CD helpers from `File_Exists` (`0x8005C4F0`) through `Movie_MoveDisplayImage` (`0x8005C568`): the Psy-Q `DsSearchFile` status wrapper, `Movie_StepPlayback`, and the display-environment image move used during stream teardown. `File_Exists` returned to pure matching C after its post-terminal register-local reconstruction; the upper boundary is the `gcc_2_8_1_g8` MDEC synchronization unit |
| `src/game/sound_voice_envelope.c` | `gcc_2_8_1_g8_split` | The adjacent SPU envelope setters `SD_SetVoiceEnvelopeFromTone` (`0x8004A6F8`) and `SD_ResetVoiceEnvelope` (`0x8004A764`). Both target the same `SpuVoiceAttr` block at `D_8009B458 + 0x4C0`, select a voice from `D_80011434`, set the ADSR mask, and call `SpuSetVoiceAttr`; one copies the caller's tone envelope and the other installs the zero/default envelope. The following voice-stop helper changes profile to `gcc_2_8_1_g0_split` |
...
| `src/game/mdec_sync.c` | `gcc_2_8_1_g8` | Movie fade-out request `Movie_StartFadeOut` (`0x8005C5C4`), which sets the `D_8009B064` flag rather than the `D_8009B062` completion latch, and the contiguous bounded wait/reset helper (`0x8005C5D4`) that clears that latch |
...
| `src/game/duel_effect_object_commands.c` | `gcc_2_8_1_g8` | Five contiguous secondary duel-effect stream commands from `0x800389C4` through `Text_CloseChoice` (`0x80038B08`): clear a channel flag, update either half of the current stream word, unlock a defeated duelist, and retire the channel's display object. `Text_UnlockDuelist` consumes command `0x19`'s duelist ID and writes its campaign/Free Duel flags; `Text_CloseChoice` handles command `0x1A` by releasing the choice object and yielding in state 2. They occupy entries 22 through 26 of `D_80090EAC`, share the typed `DuelEffectChannel` stream/state record, and are bounded above by `text_control_commands.c` at `gcc_2_8_1_g8_split` |
| `src/game/duel_effect_command.c` | `gcc_2_8_1_g8_split` | Entries 1 through 12 of the secondary text-command table `D_80090EAC` together with `func_80038024`, the helper they share, which is not in the table itself -- thirteen contiguous definitions from `0x80038024` through `0x800384E4`. Entry 0 is `func_80037DA4`, which stays a candidate and is not in this unit. Each takes the text channel and reads its operands from the channel's live stream. All thirteen are declared in `duel_effect_command.h`, which already covered them while they sat in four sources. #3859 moved `func_80037DA4`, `func_800382A8` and `func_80038334` to `src/candidates/` and the unit was split around them; #4028 and #4065 matched the latter two again without register pins and returned them to `src/game/` as single-function sources, which made the run contiguous once more without rejoining it. This is that rejoin. Entry 13, Text_StartCampaignDuel, is next in both the table and the image but only builds at `gcc_2_8_1_g0` and stays its own unit. Bounded below by `func_80037DA4` and above by `func_80038530`, both still in `src/candidates/` |
...
| `src/game/duel_card_effects.c` | `gcc_2_8_1_g8_split` | Nine contiguous field/card-effect functions from `DuelEffect_ApplyLifePointRecovery` (`0x800250C8`) through `DuelEffect_ApplyStatPenalty` (`0x80025D30`): eight adjacent application handlers in `gDuelEffect_apfnGroupHandler` plus `DuelEffect_UpdateRevealCard`, the display callback installed by `DuelEffect_ApplyDarkPiercingLight`. The first five cover LP recovery/damage, monster removal, Stop Defense and board destruction; `DuelEffect_ApplyRaigeki`, `DuelEffect_ApplyDarkPiercingLight` and `DuelEffect_ApplyStatPenalty` sweep the selected field grid through the same `D_8009B220`, `D_8009B20C`, `D_8009B260`, `D_8009B17C`, `D_800907D8` and `D_801A7AD8` state. The unit owns the LP amount tables in `.sdata` and the shared `DuelFieldEffectObject` contract. Private 2D and flat same-symbol grid views preserve the transition handlers' original addressing while the earlier handlers keep the canonical flat declaration. The preceding `func_80025028` uses `gcc_2_8_1_g8_split_comm`; the following `DuelEffect_StartSwords` uses `gcc_2_8_1_g8` |
...
| `src/game/display_object_interpolation.c` | `gcc_2_8_1_g0` | The complete contiguous three-function position-interpolation primitive unit: `DisplayObject_SavePosition` (`0x80043178`) copies the live output pair into the saved source pair, `DisplayObject_InterpolatePositionCosine` (`0x8004318C`) eases that source toward a target through their midpoint, and `Widget_SlideSine` (`0x80043230`) handles signed-phase dialog/widget entry and exit slides. The former save helper was recorded alone at g8 and is byte-identical at the g0 profile required by both interpolators. Unmatched `func_80042C08` bounds the unit below; `Main_LoadBootPackageStage` begins the unrelated file-loading runtime above |
...
| `src/game/input_pad1_backup.c` | `gcc_2_8_1_g8_split_comm` | The temporary pad-2 swap, two contiguous functions: `Input_BackupPad1AndUsePad2` (`0x8003CDF8`) saves the three pad-1 masks and copies the pad-2 masks over them, and `Input_RestorePad1FromBackup` (`0x8003CE48`) puts them back. Both are declared in `input.h` and the unit owns the COMMON definitions of the pad-1 masks and their backups. The two former sources carried four per-file volatile guards between them, and the merged unit builds byte-identically with none of them, so it defines none -- which left `GINPUT_PAD1_BACKUPS_ARE_VOLATILE` and `GINPUT_PAD2_REPEAT_IS_VOLATILE` with no definer anywhere and those two arms are gone from `input.h`. Bounded below by `input_pads.c` and above by `save_data_payload.c`, both on a profile change |
...
| `src/game/mem_card_dialog_load_save.c` | `gcc_2_8_1_g8_split` | 5 contiguous functions: `MemCardDialog_SetMessage` (`0x8003E46C`), `MemCardDialog_UpdateLoad` (`0x8003E490`), `MemCardDialog_StepLoad` (`0x8003E7D4`), `MemCardDialog_StepLoadUnprompted` (`0x8003E80C`), `MemCardDialog_UpdateSave` (`0x8003E854`). The unit owns the load and save jump tables in the rodata run from `0xC10`. #3859 had moved `MemCardDialog_UpdateSave`, which matched only through a pinned register and a private wide alias, to `src/candidates/`; it is back as pure C (#5), with the state-10 save-sequence increment written as wrapping u32 arithmetic. |
...
| `src/game/mem_card_driver.c` | `gcc_2_8_1_g8_split` | Eighteen contiguous low-level memory-card driver functions from `MemCard_ClearIOEvents` (`0x80043D48`) through `MemCard_DoLoadDirectory` (`0x80044608`): event reset/poll and LIBCARD startup, event close/open and bounded card-clear retry, the common request gate, seven request wrappers, directory enumeration/free-space/name lookup, and the directory-load state machine that consumes them. `MemCard_BeginRequest` seeds the channel/request/retry/load state used by every wrapper; all request paths clear and poll the same two event-handle banks; `MemCard_DoLoadDirectory` drives `_card_info`, `_card_clear`, `_card_load`, `MemCard_FindFiles` and `MemCard_CalcFreeBlocks` over that state. The former event-helper, close-event, begin-request, card-info and directory-helper objects are byte-identical under `gcc_2_8_1_g8_split`. The unrelated frontend loop bounds the run below, while the separately compiled request dispatcher `func_80044838` bounds it above |
| `src/game/mem_card_io_result_callbacks.c` | `gcc_2_8_1_g0_split` | The three matched memory-card I/O result callbacks, contiguous from `MemCard_SetIOResultCompleteCB` (`0x80044CFC`) through `MemCard_SetIOResultErrorCB` (`0x80044D20`): each stores its own result code in `gMemCard_nIOResult` and returns zero, and all three are declared in `mem_card_io_result_callbacks.h`, which the three former single-function sources already shared. Two were recorded at `gcc_2_8_1_g0` and one at `gcc_2_8_1_g0_split`, and every member compiles to an identical object at the split profile -- the other direction does not, so the split addressing is a requirement here rather than recorded history. The source name is the original one from before #3859 took `MemCard_SetIOResultNewCardCB` out of it, which is what scattered the other three into single-function sources. That fourth callback now lives in [`mem_card_set_io_result_new_card_cb.c`](../src/game/mem_card_set_io_result_new_card_cb.c) and bounds the unit above |
...
| `src/game/sound_output.c` | `gcc_2_8_1_g8` | The complete 18-function output/control run from `func_80046DE8` (`0x80046DE8`) through `func_80047458` (`0x80047458`). It resets and shuts down output state, selects output modes, pumps per-frame sound work, constructs command variants, dispatches packed BGM/effect requests, and bridges sequence-state setters. The two former candidate boundaries are matching pure C again, restoring the original semantic run up to the different-profile `func_80047480.c`. |
...
| `src/game/sound_voice_selection.c` | `gcc_2_8_1_g0` | Twelve contiguous voice-selection, attribute-publication, and key-off functions from `func_80047864` (`0x80047864`) through `func_80047F38` (`0x80047F38`). The run initializes per-slot SPU attributes, advances envelopes and timers, changes selections, starts batches, normalizes sound IDs, performs bounded per-mask and all-slot key-off retries, removes active slots, resolves tagged lookup codes, and filters voice flags through the same `g_SDValue` arrays. The former candidate boundaries are now matching pure C; the unit ends at the following `sound_effect_voices.c` profile change. |
...
| `src/game/sound_transfer_lifecycle.c` | `gcc_2_8_1_g0` | 2 contiguous functions: `0x80049640`, `SD_Term` (`0x80049694`). What remains after #3859 moved `SD_VabOpenHead`, which matched only through pinned registers or inline asm, to `src/candidates/`. |
...
| `src/game/sound_output_state.c` | `gcc_2_8_1_g0` | Seven contiguous sound-output state functions from `func_8004503C` (`0x8004503C`) through `SD_ClearBusyFlag` (`0x8004544C`): decoded CD-level measurement, duplicate-command suppression, two request paths, and busy-state completion. The unit was split after #3859 moved three routines to candidates; their later pure-C promotions restore the complete semantic and executable-order run. `func_80045054` retains its measured volatile pointer view through a same-symbol alias while the surrounding functions use the ordinary shared declaration. |
| `src/game/sound_runtime.c` | `gcc_2_8_1_g0` | The complete three-function command pump from `SD_EnqueueCommand` (`0x80045BE8`) through `SD_UpdateRuntime` (`0x80045F3C`). The queue writer feeds the fade updater and per-frame dispatcher, which consumes the first 0x30-byte command, applies its output/secondary transitions, compacts the remaining queue, and invokes the surrounding different-profile command executor. The former `SD_UpdateFades` and `SD_UpdateRuntime` split sources are matching pure C again, restoring the contiguous run between `func_80045514.c` and `func_80046294.c`. |
...
| `src/game/sound_voice_data.c` | `gcc_2_8_1_g0` | 5 contiguous functions: `func_80048A28` (`0x80048A28`), `0x80048C0C`, `0x80048C70`, `0x80048D08`, `func_80048F14` (`0x80048F14`). #3859 had moved `func_80048A28` and `func_80048F14`, which matched only through pinned registers or inline asm, to `src/candidates/`; both are back as pure C (#5). |
| `src/game/sound_secondary_reset.c` | `gcc_2_8_1_g0` | Low-level state query (`0x800498BC`) and secondary-state reset (`0x800498F8`) |
| `src/game/sound_secondary_playback.c` | `gcc_2_8_1_g0` | Five contiguous secondary playback lifecycle functions from sequence-input staging `func_80049A64` (`0x80049A64`) through release `func_80049CB0` (`0x80049CB0`), totaling `0x294` bytes. The two start paths initialize the shared stream cursor and tracks before selecting immediate or parameterized playback; the following stop/reset paths rebuild voice tables or release the staged sequence. #3859 split the run when `SD_PlaySequence` (`0x80049AF4`) still required pinned registers or inline assembly; its pure-C promotion restores the complete unit. A same-symbol `SD_StartSequenceTracks_no_arg` alias preserves `func_80049BAC`'s measured no-argument call view while `SD_PlaySequence` uses the canonical declaration. The g8_split `func_80049920` and `func_80049CF8` units fix both boundaries. |
| `src/game/func_80049EC8.c` | `gcc_2_8_1_g0` | 3 contiguous functions: `0x80049EC8`, `0x80049F10`, `0x80049F50`. Split out of `src/game/sound_secondary_playback.c` after #3859 moved `SD_PlaySequence`, `func_80049CF8` and `func_80049DD8`, which matched only through pinned registers or inline asm, to `src/candidates/`. |
| `src/game/sound_secondary_commands.c` | `gcc_2_8_1_g0` | Four contiguous secondary sequence functions from `func_8004B49C` (`0x8004B49C`) through `SD_SequenceTimerCallback` (`0x8004B734`), totaling `0x3B8` bytes. The first three apply control-change, program-change, and pitch-bend messages to the shared channel/object state. The root-counter callback then advances that same sequence state eight times per interrupt, periodically runs object and track maintenance, and invokes the installed callback. #3859 separated the timer callback when it still required pinned registers or inline assembly; its pure-C promotion restores the complete run. `func_8004B374` below requires `gcc_2_8_1_g8_split`, while `func_8004B854` above begins the event-lifecycle unit at that same different profile. |
