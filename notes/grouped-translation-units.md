...
| `src/game/file_transfer_runtime.c` | `gcc_2_8_1_g8_split` | 4 contiguous command-completion callbacks from `0x800140A0` through `func_80014220` (`0x80014220`), all sharing the transfer state word and event-5 retry/event-2 completion protocol. |
| `src/game/func_80014294.c` | `gcc_2_8_1_g8_split` | 9 contiguous functions: `0x80014294`, `0x80014308`, `0x80014390`, `File_ActivateTransfer` (`0x800143DC`), `0x800144B8`, `0x8001455C`, `0x80014A5C`, `0x80014B30`, `0x80014C40`. The run follows the four command-completion callbacks in `src/game/file_transfer_runtime.c`. |
...
| `src/game/mem_card_dialog_load_save.c` | `gcc_2_8_1_g8_split` | 4 contiguous functions: `MemCardDialog_SetMessage` (`0x8003E46C`), `MemCardDialog_UpdateLoad` (`0x8003E490`), `MemCardDialog_StepLoad` (`0x8003E7D4`), `MemCardDialog_StepLoadUnprompted` (`0x8003E80C`). This load TU remains unchanged. After its #3859 demotion, `MemCardDialog_UpdateSave` is now matched without its register pin in standalone `src/game/mem_card_dialog_update_save.c`, not as a fifth member of this TU. |
| `src/game/mem_card_dialog_runtime.c` | `gcc_2_8_1_g8_split` | Nine contiguous memory-card dialog functions from save-step callback `MemCardDialog_StepSave` (`0x8003EE90`) through `MemCardDialog_Request` (`0x8003F758`): the save callback and empty table slot, trade save operation, modal object motion, channel setup, request/dialog dispatcher, and poll/start/request API. The callback forwards to the standalone matched `MemCardDialog_UpdateSave` in `mem_card_dialog_update_save.c`; the dispatcher invokes it and `MemCardDialog_UpdateTradeSave` through `D_80090F9C`, while the public API seeds the shared buffer/size/file/channel state. The former save/empty object and request API object are byte-identical at g8_split. The standalone save state machine fixes the lower boundary; the unit owns the trade-save switch table at rodata offset `0xCD8`, and the alignment word at `0xCEC` fixes the upper object boundary before the save-transfer runtime's table at `0xCF0` |
