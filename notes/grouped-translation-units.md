...
| `src/game/sound_voice_selection.c` | `gcc_2_8_1_g0` | Eight contiguous voice-selection and key-off functions from `0x800478EC` through `0x80047CC4`: per-slot envelope/timer cleanup, selection changes, batch starts, sound-ID normalization, bounded SPU key-off retries, and active-slot removal. The run shares `g_SDValue`'s voice arrays and is bounded below by the `func_80047864` candidate and above by the `func_80047DB0` candidate. |
| `src/game/sd_key_off_voice_slots.c` | `gcc_2_8_1_g0` | 2 contiguous functions: `SD_KeyOffVoiceSlots` (`0x80047EC4`), `0x80047F38`. Kept separate from `src/game/sound_voice_selection.c` because the `func_80047DB0` candidate occupies the exact 0x114-byte intervening range immediately before this pair. |
