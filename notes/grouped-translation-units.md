...
| `src/game/movie_frame_pipeline.c` | `gcc_2_8_1_g8` | Three contiguous movie-stream functions: `Movie_StopStream` (`0x8005BB7C`), `Movie_DecodeAndPresentFrame` (`0x8005BE3C`), and `Movie_WaitAndDecodeFrame` (`0x8005BFC8`). What remains after #3859 moved `func_8005C1F4`, which matched only through pinned registers or inline asm, to `src/candidates/`. |
...
| `src/game/mem_card_io_result_callbacks.c` | `gcc_2_8_1_g0_split` | The three matched memory-card I/O result callbacks, contiguous from `MemCard_SetIOResultCompleteCB` (`0x80044CFC`) through `MemCard_SetIOResultErrorCB` (`0x80044D20`): each stores its own result code in `gMemCard_nIOResult` and returns zero, and all three are declared in `mem_card_io_result_callbacks.h`, which the three former single-function sources already shared. Two were recorded at `gcc_2_8_1_g0` and one at `gcc_2_8_1_g0_split`, and every member compiles to an identical object at the split profile -- the other direction does not, so the split addressing is a requirement here rather than recorded history. The source name is the original one: `src/candidates/func_80044D34.c` says in its own header comment that it was `src/game/mem_card_io_result_callbacks.c` before #3859 took `MemCard_SetIOResultNewCardCB` out of it, which is what scattered the other three into single-function sources. That fourth callback is still a candidate and bounds the unit above |
