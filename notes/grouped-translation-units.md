...
| `src/game/movie_stream_requests.c` | `gcc_2_8_1_g0_split` | Two contiguous movie request constructors totaling 0x168 bytes: `func_8005C388` (`0x8005C388`) selects a configured stream range and converts its accumulated sector offset through `CdlLOC`, while `func_8005C464` (`0x8005C464`) resolves a named file and requests its sector-aligned extent. Both finish in `func_8005B8A0` and share the direct `file_cd_helpers.h` conversion ownership. G8 movie-frame code below and G0 file/CD helpers above fix the boundaries. |
