...
and in resident `card_list_render_deck_box_stats.c`. The only constant with
that value is
...
`DisplayObject_RenderSpriteSheet` in `src/game/display_object_render_sprite_sheet.c`.
   ...
   *(u16 *)(object + 8) = flags;
   ...
   renderer called from `display_object_render_sprite_sheet_list.c`. At `0x800416F0` it loads the flag
