...
| Module | Archive evidence | Loaded image range | External symbols | Confidence |
|---|---|---|---|---|
...
The loaded image ranges include each module's headers and data as well as
its MIPS text; they are not instruction extents. The WA package ranges also
include phases copied to other destinations, so the narrower module-image
ranges above are not complete package manifests.
...
| Egypt overworld | `File_RequestEgyptOverworldPackage` | `0x1FD9`, `0x9E`; first sector `+0x9E` when story flag `0x47` is set | `8153-8311`, `8311-8469` |
...
| Campaign scene loader | sectors `7767-7816` | `Campaign_LoadScenePackage` requests the package; matching callback `Campaign_LoadScenePackageStage` accounts for the image, palette, event-script, and dialog-portrait phases | No phase is copied directly to a callable module slot |
