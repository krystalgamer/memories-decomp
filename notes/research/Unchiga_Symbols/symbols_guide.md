...
| `SD_SEPlayFull` | 0x8003FEE0 | Convenience wrapper: plays an effect at full volume — always `SD_SEPlay(id, SD_SE_VOLUME_MAX)`. |
...
| `MainMenu_StartValueSetup` / `MainMenu_FinishValueSetup` (module: main_menu) | 0x80180FD8 / 0x80181E30 | Creates and tears down the adjacent dual-value setup screen, snapshotting two caller values on entry and writing both back on exit. |
...
| `Password_RefreshDigitDisplay` (module: password) | 0x80169C30 | Re-encodes all eight entered digits through the glyph table, terminates the string, and rebuilds the digit text object after initialization or a value change. |
| `Password_RefreshStarchipDisplay` (module: password) | 0x80169D10 | Copies the saved starchip balance into the dynamic-number staging word and rebuilds the slot-3 balance text box at screen entry or after a purchase. |
| `Password_CreateMessageBox` (module: password) | 0x80169D84 | Replaces text-box slot 0 with the requested password message in a fixed 160×64 rectangle; caller flags select extra box setup and display flag `0x8`. |
