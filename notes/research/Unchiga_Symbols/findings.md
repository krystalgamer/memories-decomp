...
| F113 | advancing a page (X) | RESOLVED: FA dispatches `Text_StartPageWait`, which enters state 4 with no operand. State 4 (`func_800375A4`) creates the shared choice/page cursor, waits for Cross/Square or the Square-held countdown, plays SE 0x0B, destroys the cursor and enters state 2. State 2 (`func_8003741C`) clears active glyph/effect entries and resets the line/cursor fields before returning to parsing. The observed text-pointer moves across pages are this exact state-machine path. | `Text_StartPageWait`; shared state callbacks remain address-based | APPLIED |
| F114 | text control bytes | RESOLVED from the matching `D_80090F18` dispatch and retail string 1350: F0-F5 extend glyph codes with one byte; F6 reads display-effect id+flags; F7 reads one state byte; F8 reads a secondary opcode then handler-defined operands; F9 reads a flag u16 and, except for writes, a target u16; FA has no operand; FB is variable choice setup/selected jump; FC/FD read u16 stream offsets; FE is newline and choice-line completion; FF pops a nested stream and marks done only below depth zero. Retail examples (`F8 10 03 80 87`, `F8 15 A0`, `F9 6E40 FD6E44`, `FC 5A12`, `FD 8B12`, `FB 02`, `FB 80 DE15`) independently confirm the variable forms. | `Text_ExtendGlyphCode`, `Text_SetStateFromStream`, `Text_DispatchSecondaryCommand`, `Text_HandleCampaignFlagCommand`, `Text_StartPageWait`, `Text_PushStreamOffset`, `Text_NewLine`, `Text_EndStream`, `Text_HandleDisplayEffectCommand` | APPLIED |
...
| F123 | `0x80010000..0x80010034` (14 words) | RESOLVED/REJECTED NAME: the original eight-word sample was incomplete. Full consumer coverage separates four cohorts: three general/model payload bases; four paired MODEL-loaded module slots; four paired data arguments passed to those modules; and three SU-specific pointers, the last of which is the interior address `0x80185CD4`. The 17-stage MODEL callback consumes exactly one `0x114`-sector record and proves the module/data distinction: stages 7-10 select position variants for `0x8013B000`/`0x8017B000`, stages 11-12 select primary modules at `0x8013A000`/`0x8017A000`, and `0x80135000`/`0x80175000` plus `0x80136000`/`0x80176000` are passed as data arguments. Physical contiguity is real, but one `gMain_apLoadArena` array is not: ownership is cross-subsystem and not every word is a load destination. | (`gMain_apLoadArena` rejected; keep address-based cohort members) | REJECTED |
...
| # | address | what we proved | proposed name | status |
|---|---|---|---|---|
| F132 | WA_MRG.MRG +0xF92BD4 / +0xFBDBD4 | APPLIED: two byte-identical `0x2808`-byte copies, SHA-256 `842e022e4a4bb2e5ecf633f5c5d80e3a041ab837cd3bbd65d01b67783a4d45a4`. Each contains seven checked `NameEntryStarterDeckPool` records: `{u16 draw_count; u16 weights[722]; u8 padding[18];}`, size `0x5B8`. Counts are 16,16,4,1,1,1,1; all row sums are 2048 and all tails are zero. The following pointer lists are identical: seven `0x5B8`-spaced pointers from `0x8016ABD4` through `0x8016CE24`, then null. Matching source now uses the typed null-terminated list. Retail category analysis remains four monster ATK+DEF bands, Dark Hole/Raigeki, six terrains, and 28 Equips. | `gNameEntry_aStarterDeckPools`, `gNameEntry_apStarterDeckPools` | APPLIED |
...
| F135 | `0x8016AA6C` (module) | Matching module main clears 0x801D0000..+0x3000, runs name entry and starter-deck construction, then writes `(gMain_dwVBlankTick << 8) \| XOR(SaveDataState.player_name_sjis[0..11])` to `SaveDataState.duelist_code`, substituting `rand() << 8` only while the result is zero. Matching save comparisons, two-save callers, FM-Online's bypass, and the credits modulo-five use establish the field contract; generation guarantees nonzero, not uniqueness. | `NameEntry_Main`; `gMain_dwVBlankTick`; `gSaveData_dwDuelistCode` | APPLIED |
...
| F141 | SLUS 0x81350 (RAM 0x80090B50) | The five credits "SECRET NO." codes as 5 x {u16 high4, u16 low4}: 86985631, 37689434, 73153736, 92886423, 53297534. `Main_RunCredits` selects entry `gSaveData_dwDuelistCode % 5` and shows string 0x23. | `gCredits_awSecretNumbers` | APPLIED |
| F142 | WA_MRG 0xB48000 / 0xB60800 | RESOLVED: game font sheet, shared UI images, palette block and colour ramps occupy the first 51 sectors of a 54-sector package. Matching `Main_LoadBootPackageStage` accounts for all four phases: 48 sectors fill VRAM `(640,0)-(831,255)`, two upload at `(512,248,256,8)`, one supplies the eight colour-ramp rows at `(640,232,16,8)`, and the last three load module ID `0x16` at `0x80168000`. That module contains the "SOFTWARE TERMINATED / CONSOLE MAY HAVE BEEN MODIFIED" warning; `Main_RunBootSequence` calls its BIOS-region initializer at `0x801680F4`, polls its 20-state controller at `0x80168160`, then loads the main menu. The earlier `Boot_LoadUiBlock` proposal was too narrow for the outer function. | `Main_LoadBootPackageStage`, `Main_RunBootSequence` | APPLIED |
| F143 | WA_MRG 0xF35000 | RESOLVED: the campaign package loads `0xF000` bytes to `0x80100000`. `Campaign_LoadScenePackage` consumes exactly 25 records of `0x980` bytes: 48x48 8bpp image (`0x900`) plus 64-entry CLUT (`0x80`), bank SHA-256 `3b3898fdf0d74a0ed8d86ca4faac871621747dfe85977db4f5697c18725dcccf`. It uploads a 5x5 atlas and CLUT rows; F6 ids `0x41..0x59` map through `id-0x41` to the same UV/CLUT geometry. The final `0x280` phase bytes are not consumed and exactly match the prefix of another `0x980` portrait record at WA `0xF63D80`, establishing sector-rounded portrait spill rather than a table. The proposed `gText_aDialogPortraits` name is not applied to `D_80010000`, which is a reused pointer object. | `Campaign_LoadScenePackage`, `Campaign_LoadScenePackageStage`; portrait constants in `campaign_scene_package.h` | APPLIED |
...
| F148 | `0x80090B50` | The five credits "SECRET NO." codes as 5 x {u16 high4, u16 low4}: 86985631, 37689434, 73153736, 92886423, 53297534. Referenced only from the credits loop (`Main_RunCredits`, lui/addiu pair at 0x8002DB5C/0x8002DB60), which selects entry `gSaveData_dwDuelistCode % 5`. | `gCredits_awSecretNumbers` | APPLIED |
...
| F154 | `0x8005B64C` `select_reverb_preset` | RESOLVED: matching source and the phase stepper prove that the selector is the monotonically increasing stage index for the sole 115-sector `SU.MRG` main-menu request. The five programmed lengths sum exactly to the request and map sectors 0-64 and 64-96 to two VRAM image uploads, 96-98 to the palette staging buffer, 98-114 to the executable at `0x80180000` after the palette upload, and 114-115 to the `D_801AF800` display-resource bank consumed by the loaded overlay. This is the main-menu analogue of `Duel_LoadPackageStage`, not a generic CD preset selector and not an SPU routine. | `MainMenu_LoadPackageStage` | APPLIED |
...
| F157 | `0x8002CB80` `Duel_CalcGuardianStarMatchup` | The old hedge in the caller now at `src/game/duel_battle_stats.c` is settled: this IS the guardian-star wheel, not a scroll helper. The TEA-Online guardian-star patch overwrites this exact 116-byte function with its own 96-byte matchup routine plus a 16 x u16 mask table, independently identifying the same behavior. The matching implementation now lives in `src/game/duel_calc_guardian_star_matchup.c`. | (name unchanged; hedge resolved) | CONFIRMED |
