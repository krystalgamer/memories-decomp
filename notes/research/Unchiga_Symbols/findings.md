...
| F142 | WA_MRG 0xB48000 / 0xB60800 | Game font sheet: 256x168 4bpp, 128-byte rows, low nibble = left pixel, with a 16-entry CLUT at 0xB60800. Two grids: 8x12 cells for rows 0..71 and 16x16 cells for rows 72..167. Loaded as part of one 54-sector boot block by `func_80043960` -> `func_80014E1C(0,0,0x1690,0x36,func_80043328)`, which also carries the UI icon sheet (0xB50000) and a 0x1000-byte CLUT block (0xB60000) that `func_80043328` uploads to VRAM at (512,248,256,8). | `gText_aFontSheet`, `Boot_LoadUiBlock` | PENDING |
...
| # | address | what we proved | proposed name | status |
|---|---|---|---|---|
| F156 | `0x801D071C` | The per-duelist Free Duel win/loss record: 40 x {u16 wins, u16 losses}, capped at 999. Read and incremented from the free-duel disc module (reader at WA in-file 0xF6D15C, incrementer at 0xF6D2A8) as `0x801D071C + 4*(row*5+col)`. This is the increment site F73 and F121 describe but leave unlocated. Extent corroborates it: 40 x 4 = 160 bytes ending at exactly 0x801D07BC, where `gDuel_awRecentCardDrops` begins, with `gFreeDuel_dwUnlockedDuelists` (0x801D06F4) just below. | `gFreeDuel_aDuelistRecords` | APPLIED |
