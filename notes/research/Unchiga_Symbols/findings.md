...
| F72 | `0x8009B165` (1 byte) | Winner side for the F23 block: used as index into `g_DuelSavePtrs` (winner gets wins++ at ptr+0x518, loser = index^1 gets losses++ at ptr+0x51A). Live traces observed 0 on a player win and 1 from duel mode through the end-credit latch after a CPU win against Duel Master K. | `gDuel_bWinnerSide` | APPLIED |
