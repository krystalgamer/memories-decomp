...
| F72 | `0x8009B165` (1 byte) | Winner side for the F23 block: used as index into `g_DuelSavePtrs` (winner gets wins++ at ptr+0x518, loser = index^1 gets losses++ at ptr+0x51A). The consumed CPU-win trace recorded 1 from duel mode through the end-credit latch, but it began after a previous loss and therefore does not rule out stale state. The role is established; 0=player / 1=opponent remains high confidence pending the player-win control. | `gDuel_bWinnerSide` | APPLIED |
