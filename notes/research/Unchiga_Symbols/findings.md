...
| F152 | `0x800E9EB4` | Candidate per-file LBA table (`gFile_anLba`, MOVIE.STR at index 3) reported by the FMV tool analysis. NOT verifiable statically: the eight words are zero in SLUS_014.11, so the table is filled at runtime from the ISO directory. Needs a live trace before naming. | (`gFile_anLba`, unconfirmed) | HOLD |
