...
| F135 | `0x8016AA6C` (module) | Matching module main: clears 0x801D0000..+0x3000, printf, NameEntry_Init, then `{call_four_update_funcs; rand();} while (NameEntry_PollCompletion()==0)`, then F134, then writes `(rand_or_D_8009B09C<<8) \| XOR(name bytes 0x801D060C..+11)` (re-rolled until non-zero) to 0x801D0534. The function identity is applied; the save-ID field remains open. | `NameEntry_Main` (APPLIED); 0x801D0534 = `gSave_dwGameID`? | HOLD |
