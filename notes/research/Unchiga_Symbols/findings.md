...
| # | address | what we proved | proposed name | status |
|---|---|---|---|---|
| F129 | `0x80021EDC` in `Duel_MaybeEndOfDuel` | APPLIED: `DuelResultDisplayState` now covers the complete `0x40`-byte result record. `+0x37` is the page index cycled modulo three and consumed by `Duel_ShowResultPage`; `+0x38` is rank tier `0..4`; `+0x39` is the TEC-axis flag; `+0x3A` is `starchip_prize = tier + 1`; and `+0x3C` is the selected card drop. The award adds the prize to `SaveDataState.starchips` at `+0x5E0`, clamps to `SAVE_DATA_STARCHIP_MAX` (`999999`), then awards the stored card. The same balance is the established `gLibrary_dwStarchips` used by the Password shop. | `Duel_ShowResultPage`; typed result/save fields | APPLIED |
