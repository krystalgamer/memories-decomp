...
| F82 | `0x80168C7C` (module) | Free-duel screen tick: dialog-open path (flag 0x20 of 0x801690A4) else cursor tween tick (0x80168A9C), scrollbar update (0x80168004), then input: pad-held word `0x8009B3A4` bits 0x2000 RIGHT / 0x8000 LEFT / 0x4000 DOWN / 0x1000 UP move the pending cell; newly-pressed word `0x8009B398` has 0x20 cancel (SE 8) and 0xC0 confirm: cell (0,0) opens Build Deck (mode 6), any other available cell checks the 40-slot deck at 0x801D0200 (empty slot -> SE 9 + dialog), else SE 0x30, init_800eb26c_fields_from_args(-1, row*5+col, 0x6000, 0x6000), mode byte 0x8009B26C = 3 (duel start). | `FreeDuel_UpdateScreen` | APPLIED |
...
| F102 | `0x8009B3A4` / `0x8009B398` (pad words) | CORRECTED by matching `Input_UpdatePads`: 0x8009B3A4 is pad 1's held mask, with DPAD in the high nibble (0x1000 UP, 0x2000 RIGHT, 0x4000 DOWN, 0x8000 LEFT). 0x8009B398 is pad 1's newly-pressed mask, including 0x20 cancel/Circle and 0x40/0x80 confirm/X. Their adjacent halfwords at +2 are the corresponding pad 2 masks. | held/pressed global family | CONFIRMED |
...
| F109 | `0x8009B394` (pad word for the text system) | Matching `Input_UpdatePads` proves this is pad 1's repeat mask: every newly pressed bit is included immediately, then held bits recur through per-button timers. Its adjacent halfword at 0x8009B396 is the pad 2 repeat mask. | repeat global family | CONFIRMED |
...
| F130 | `D_801D5708` (u8[16]) | Text colour-slot table, not a "state bar": `func_80038498` maps text bytes 0x80..0x8F through `D_801D5708[b & 0xF]`; `func_8003C4E0` seeds 5 slots to 4 / active 0 / other 2. The site's "Result color" patch replaces the `sll/sra` at 0x80021C7C with `li a1,(COM<<8)\|YOU; sw a1,D_801D5708` and documents the values: 0 white, 1 yellow, 2 blue, 3 green, 4 grey, 5 orange, 6 red. Result screen draws YOU/COM numbers through slots 0/1. | `gText_abColorSlots` | PENDING |
...
| # | address | what we proved | proposed name | status |
|---|---|---|---|---|
| F132 | WA_MRG.MRG +0xF92BD4 / +0xFBDBD4 | Starter-deck pool table, read straight off the disc (WA_MRG = LBA 10102): 7 rows x 1464 B = {u16 cards-to-draw, u16 weight[722], 18 B pad}; counts 16,16,4,1,1,1,1 (=40), every row's weights sum to 2048. Two byte-identical copies, in-file sectors 0x1F20+0x3D4 and 0x1F76+0x3D4; the site's tool writes both. Followed by a NULL-terminated list of 7 pointers 0x8016ABD4+0x5B8*k, which fixes the module base at 0x80168000. | `gNameEntry_aStarterSets` (0x8016ABD4), `gNameEntry_apStarterSets` (0x8016D3DC) | PENDING |
...
| F134 | `0x8016A930` (module) | Starter-deck generator, disassembled from the disc module: per set, `n` draws of threshold `(rand()&0x7FF)+1` walked over weight[0..719] (NOT 722), one extra rand() per step; hit -> if copies[i]<3 append i+1 to `gDuel_awPlayerDeck` and `Library_UpdateCardUsedFlag(i+289)`, else redraw. So: 3-copy cap, ids 721/722 undrawable, short deck if a row sums < 2048. Same walk as Duel_SelectCardDrop. | `NameEntry_BuildStarterDeck` | PENDING |
| F135 | `0x8016AA6C` (module) | Module main: clears 0x801D0000..+0x3000, printf, NameEntry_Init, then `{call_four_update_funcs; rand();} while (func_80169C08()==0)`, then F134, then writes `(rand_or_D_8009B09C<<8) \| XOR(name bytes 0x801D060C..+11)` (re-rolled until non-zero) to 0x801D0534. Likely the EXE's fixed-address entry for the name-entry screen (F100 left it open). | `NameEntry_Main`; 0x801D0534 = `gSave_dwGameID`? | PENDING |
...
| F137 | WA_MRG 0xB85000 / 0xB87800 (+0x75800 per terrain) | Duel data block tables: equip compatibility at +0x85000 (u16 key, u16 count, u16 members[count], 0 terminator: 34 groups, 4041 memberships, 8220 B) loaded to 0x8017A1D8; packed fusion table at +0x87800 (u16 index[723] then 5-byte groups of two 10-bit partner/result pairs) loaded to 0x8017C2D8. All 7 terrain copies identical. The TEA-Online tools use offsets 0x800 higher and therefore misparse a stock US file. | `gDuel_awEquipTable`, `gDuel_aFusionTable` | PENDING |
...
| F150 | `0x8009151C` (file 0x81D1C) | The FMV playback length, stored as two u16 rather than one u32: `08 0C 2F 01` = 3080 sectors and 303 frames. So the game plays only 3080 of the 3460 sectors in copy 0, i.e. 303 of the movie's 346 frames. Reached through `func_8003594C` -> `lookup_speed_and_start_read` (0x8005C388) -> `func_8005B8A0` (StSetRing / StSetStream / DecDCTvlcBuild). | `gMovie_wSectorCount` / `gMovie_wFrameCount` | PENDING |
...
| F155 | `0x800117C8`, `0x80011580`, `0x800114F8` | Konami's own build paths survive in the image as plain strings: "M:/mrgSU/SU.mrg" (twice, at 0x800114F8 and 0x800117C8) and "M:/mrgSU/model.mrg". The 0x800117C8 copy is passed as the filename argument to the CD loader by `func_8005B85C`. These are the only drive-letter paths in the EXE. The duplicate at 0x800114F8 is left unnamed until its own use is traced. | `gFile_szSuMrgPath` (0x800117C8), `gFile_szModelMrgPath` (0x80011580) | APPLIED |

| # | address | what we proved | proposed name | status |
|---|---|---|---|---|
| F156 | `0x801D071C` | The Free Duel grid win/loss table: 40 x {u16 wins, u16 losses}, capped at 999. Slot 0 belongs to the Build Deck tile and slots 1–39 to the duelists. The free-duel disc module reads it at WA in-file `0xF6D15C`, forms the selected record address at `0xF6D2A8`, and stores the increment at `0xF6D2D4` as `0x801D071C + 4*(row*5+col)`. This resolves the earlier F75 search and supplies the exact write site behind F121. Extent corroborates it: 40 x 4 = 160 bytes ending at exactly 0x801D07BC, where `gDuel_awRecentCardDrops` begins, with `gFreeDuel_dwUnlockedDuelists` (0x801D06F4) just below. | `gFreeDuel_aDuelistRecords` | APPLIED |
| F157 | `0x8002CB80` `Duel_CalcGuardianStarMatchup` | The hedge recorded in src/func_8001EE44.c is now settled: this IS the guardian-star wheel, not a scroll helper. The TEA-Online guardian-star patch overwrites this exact 116-byte function with its own 96-byte matchup routine plus a 16 x u16 mask table, i.e. an independent party identified the same function the same way. The unit FILE is still named src/calc_wrap_scroll_delta.c (unit names are a separate namespace); the symbol was already correct. | (name unchanged; hedge resolved) | CONFIRMED |
