...
| `0x0814`, `0x0815` | `u8` | offset-based fields | Initialization and update/output controls set/test these bytes. `SD_EnableSecondaryPanning` clears `0x0815`, while `SD_DisableSecondaryPanning` sets it; when set, `SD_SpatializeSecondaryObject` forces centre pan instead of using its computed value. |
