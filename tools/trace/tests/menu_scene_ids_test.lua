...
--   luajit tools/trace/tests/menu_scene_ids_test.lua
...
-- any numeric ID to visible menu text.
...
local scriptPath = SCRIPT_UNDER_TEST
    or 'tools/trace/menu_scene_ids.lua'
...
local function capture(validSignature, mode)
    ...
    listener_menu_scene_ids_reset = nil
    listener_menu_scene_ids = nil
    dofile(scriptPath)
...
local normal = capture(true, 0x28)
normal:lookup(0x007, 0x80011110)
normal:lookup(0x02a, 0x80011110)
for id = 0x008, 0x00c do
    normal:lookup(id, 0x80022220)
end
normal:lookup(0x008, 0x80022220)
normal:frames(300)
assert(normal:has('status: captured five distinct menu-scene IDs'),
       'five IDs must finish after the 300-frame quiet window')
assert(normal:has(
    'summary: hits=5 unique_ids=5 '
        .. 'observed_ids=0x008,0x009,0x00A,0x00B,0x00C'
), 'the summary must sort and deduplicate IDs')
assert(normal:has(
    'id=0x008 callsite=0x80022220 mode_raw=0x28 mode=08'
), 'each row must preserve caller and normalized mode')
assert(not normal:has('id=0x007'), 'IDs below the range are ignored')
assert(not normal:has('id=0x02A'), 'IDs above the range are ignored')
assert(normal:disabled())

local limited = capture(true, 8)
for index = 1, 64 do
    limited:lookup(0x008, 0x80030000 + index * 4)
end
limited:lookup(0x008, 0x80040000)
limited:frames(1)
assert(limited:has(
    'status: maximum context count reached; partial trace follows'
), 'the context capture must stop at 64 accepted rows')
assert(limited:has(
    'summary: hits=64 unique_ids=1 observed_ids=0x008'
), 'the hit bound must not inflate the unique-ID count')
assert(limited:disabled())

local silent = capture(true, 8)
silent:frames(600)
assert(silent:has(
...
assert(silent:has('status: timed out without a menu-scene lookup'),
       'a silent run must finish with an explicit no-hit status')
assert(silent:disabled())

local broken = capture(true, 8)
broken:breakRegisters()
broken:lookup(0x008, 0x80055550)
broken:frames(1)
...
print = hostPrint
print('menu_scene_ids: all six callback-replay cases passed')
