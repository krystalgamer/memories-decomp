...
--   luajit tools/trace/tests/password_scene_ids_test.lua
...
-- any numeric ID to visible Password-screen text.
...
local scriptPath = SCRIPT_UNDER_TEST
    or 'tools/trace/password_scene_ids.lua'
...
local function capture(validSignature, mode)
    ...
    listener_password_scene_ids_reset = nil
    listener_password_scene_ids = nil
    dofile(scriptPath)
...
local normal = capture(true, 0x2a)
normal:lookup(0x0e0, 0x80011110)
normal:lookup(0x0e4, 0x80011110)
for id = 0x0e1, 0x0e3 do
    normal:lookup(id, 0x80022220)
end
normal:lookup(0x0e1, 0x80022220)
normal:frames(300)
assert(normal:has('status: captured all three Password-screen IDs'),
       'three IDs must finish after the 300-frame quiet window')
assert(normal:has(
    'summary: hits=3 unique_ids=3 observed_ids=0x0E1,0x0E2,0x0E3'
), 'the summary must sort and deduplicate IDs')
assert(normal:has(
    'id=0x0E1 callsite=0x80022220 mode_raw=0x2A mode=10'
), 'each row must preserve caller and normalized mode')
assert(not normal:has('id=0x0E0'), 'IDs below the range are ignored')
assert(not normal:has('id=0x0E4'), 'IDs above the range are ignored')
assert(normal:disabled())

local limited = capture(true, 10)
for index = 1, 32 do
    limited:lookup(0x0e1, 0x80030000 + index * 4)
end
limited:lookup(0x0e1, 0x80040000)
limited:frames(1)
assert(limited:has(
    'status: maximum context count reached; partial trace follows'
), 'the context capture must stop at 32 accepted rows')
assert(limited:has(
    'summary: hits=32 unique_ids=1 observed_ids=0x0E1'
), 'the hit bound must not inflate the unique-ID count')
assert(limited:disabled())

local silent = capture(true, 10)
silent:frames(600)
assert(silent:has(
    'no Password lookups observed; confirm interpreter CPU'
), 'a no-hit run must print the likely setup diagnosis')
silent:frames(71400)
assert(silent:has('status: timed out without a Password-screen lookup'),
       'a silent run must finish with an explicit no-hit status')
assert(silent:disabled())

local broken = capture(true, 10)
broken:breakRegisters()
broken:lookup(0x0e1, 0x80055550)
broken:frames(1)
...
print = hostPrint
print('password_scene_ids: all six callback-replay cases passed')
