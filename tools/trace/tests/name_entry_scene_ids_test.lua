...
--   luajit tools/trace/tests/name_entry_scene_ids_test.lua
...
-- any numeric ID to visible name-entry text.
...
local scriptPath = SCRIPT_UNDER_TEST
    or 'tools/trace/name_entry_scene_ids.lua'
...
local function capture(validSignature, mode)
    ...
    listener_name_entry_scene_ids_reset = nil
    listener_name_entry_scene_ids = nil
    dofile(scriptPath)
...
normal:lookup(-1, 0x80011110)
normal:lookup(0x008, 0x80011110)
normal:lookup(0x004, 0x80022220)
normal:lookup(0x004, 0x80022220)
normal:frames(300)
assert(normal:has('status: captured a low scene ID on the name-entry route'),
       'one ID must finish after the 300-frame quiet window')
assert(normal:has(
    'summary: hits=1 unique_ids=1 observed_ids=0x004'
), 'the summary must deduplicate the arbitrary in-range ID')
assert(normal:has(
    'id=0x004 callsite=0x80022220 mode_raw=0x29 mode=09'
), 'each row must preserve caller and normalized mode')
assert(not normal:has('id=0xFFFFFFFF'), 'wrapped IDs below zero are ignored')
assert(not normal:has('id=0x008'), 'IDs above the range are ignored')
assert(normal:disabled())

local limited = capture(true, 9)
for index = 1, 32 do
    limited:lookup(0x003, 0x80030000 + index * 4)
end
limited:lookup(0x003, 0x80040000)
limited:frames(1)
assert(limited:has(
    'status: maximum context count reached; partial trace follows'
), 'the context capture must stop at 32 accepted rows')
assert(limited:has(
    'summary: hits=32 unique_ids=1 observed_ids=0x003'
...
silent:frames(600)
assert(silent:has(
    'no low scene IDs observed; confirm interpreter CPU'
), 'a no-hit run must print the likely setup diagnosis')
silent:frames(35400)
assert(silent:has('status: timed out without a low-scene lookup'),
...
broken:lookup(0x001, 0x80055550)
broken:frames(1)
...
print = hostPrint
print('name_entry_scene_ids: all six callback-replay cases passed')
