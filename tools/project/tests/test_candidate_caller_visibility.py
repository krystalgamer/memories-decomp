...
PAIRS = [
    ...
    ("src/candidates/func_80040DD8.c", "func_80042188", '#include "../game/display_object_packet_submit.h"'),
]

PACKET_SUBMIT_CANDIDATES = (
    "src/candidates/func_80028B08.c",
    "src/candidates/func_80040DD8.c",
)
        ...
        for source, callee, include in PAIRS:
            ...
            with self.subTest(source=source):
                ...
                with tempfile.TemporaryDirectory(dir=REPOSITORY / "tmp") as directory:
                    probe = Path(directory) / "a/b/c" / Path(source).name
                    probe.parent.mkdir(parents=True)
                    probe_text = text.replace(include + "\n", "", 1)
                    # The raw-attribute renderer also includes unmatched.h,
                    # whose central fallback carries the same raw prototype.
                    # Remove that fallback in the negative probe so the
                    # compiler can demonstrate that both declaration paths
                    # would have to be lost before the call becomes implicit.
