...
Modes:
  ...
  (default) regenerate the index section of notes/candidates/README.md
  --check   verify the index is current and the store is well formed
"""
...
def write_index(entries):
    ...
    if current.count(INDEX_START) != 1 or current.count(INDEX_END) != 1:
        raise SystemExit(
            f"{INDEX.relative_to(ROOT)} must contain exactly one "
            f"{INDEX_START} and one {INDEX_END}"
        )
...
def main():
    ...
    if args.check:
        ...
        if write_index(entries):
            INDEX.write_text(before, encoding="utf-8")
            raise SystemExit(
                "notes/candidates/README.md is out of date; "
                "run `make candidate-index`"
            )
        print(f"candidate index: OK ({len(entries)} entries)")
        return 0
