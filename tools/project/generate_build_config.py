...
def generate(root: Path) -> tuple[Path, Path]:
    ...
    def one_segment(name: str) -> dict[str, Any]:
        found = [
            segment
        ...
        if len(found) != 1:
            raise GenerationError(f"expected exactly one {name} segment")
        return found[0]

    # The leading read-only region and the text are separate segments, because
    # Splat emits one output section per segment and orders by section_order
    # within it. Together they would put the initialized data before the text.
    main_segments = [one_segment("main")]
    text_segment = one_segment("text")
    ...
    # Everything after the text is declared as its own segment in the
    # template, so nothing is appended here.
