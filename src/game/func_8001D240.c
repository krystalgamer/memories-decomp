...
{
 if(!func_80042B98((DisplayObjectLifecycle*)o)){o->flags|=DISPLAY_OBJECT_FLAG_CLIP_TEST;if(!o->mode){o->dx=-8;o->dy=0xC0;}else{o->dx=8;o->dy=0;}}
 o->mode+=o->dx;if(o->mode<0xC0){DuelCardRecord*e=&D_801A7AD8[o->index];e->flags|=DUEL_CARD_FLAG_DEFENSE_POSITION;o->mode=o->dy;
  if(!o->mode){e->flags&=~DUEL_CARD_FLAG_DEFENSE_POSITION;if(!o->current)o->flags&=~DISPLAY_OBJECT_FLAG_CLIP_TEST;}o->active=0;o->callback=0;}
}
