#include "../../types.h"

/* SLPM-86398 build of src/game/duel_load_package_stage.c: the symbols below sit at other addresses in the
 * Japanese executable and their US names are taken there, so they are aliased
 * (config/slpm_86398/symbols.txt has the addresses). The US source is included
 * as is. */
#define D_8009B0F4_abs gJapanese_FileTransferFlags
#define D_800E9D70 gJapanese_D_800E9D70

/* Values that differ in the Japanese release; the US source names each
 * with an #ifndef default (jp_promote.REGIONAL). */
#define DUEL_PACKAGE_STAGE7_SECTORS 48

#include "../duel_load_package_stage.c"
