 ...
 *
 * The return is the fact the four callers were losing. It hands back the
 * transfer descriptor it started, with `slot` already stored in the
 * descriptor's callback_data for func_800289BC to pick up, and it has
 * published the descriptor's status through D_8009B0F4 with
 * FILE_TRANSFER_STATE_PRIMARY_ACTIVE set. All four callers declared it void
 * and ignore the value, which is why they can: the descriptor is reachable
 * without it. Nothing about the call sites changes. */
