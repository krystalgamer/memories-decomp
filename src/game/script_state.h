 ...
 * Script_ReadByte returns *D_8009B290++ for a single-byte operand; the second
 * reader returns current[0] | (current[1] << 8) after advancing two. Operands
 * are little-endian and the cursor is byte-granular. Command handlers advance
 * it by two, four or six according to how many operands they take, and
