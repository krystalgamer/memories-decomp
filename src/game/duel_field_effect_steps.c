 ...
 * Steps past the fifth clear gDuel_wCardEffectFlags and end the sweep. A card
 * sitting in the swept slot with a negative stat modifier has it cleared and
 * gets the alternate object state 5. */
void DuelEffect_ApplyCursebreaker(void)
{
    ...
    if ((s16)next < DUEL_FIELD_ROW_SIZE) {
        /* This step walks the grid flat, with the side folded into base_slot
           below, while sibling DuelEffect_ApplySwords uses the two-dimensional
           view selected by DUEL_FIELD_GRID_2D. The cast is the one place the
           two spellings meet. The single-pass loop preserves the retail grid
           and side register allocation under GCC 2.8.1. */
        do {
            grid = (u8 *)D_800907D8;
